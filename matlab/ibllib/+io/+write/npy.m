function npy(filename, D)
% io.write.npy(filename, DATA);
%
...
header = make_npy_header(dataType, shape);

fid = fopen(filename, 'w+');
fwrite(fid, header, 'uint8');
fwrite(fid, D, dataType);
fclose(fid);


end

function header = make_npy_header(dataType, shape, varargin)
% header = constructNPYheader(dataType, shape, [FortranOrder_bool, LittleEndian_bool])
