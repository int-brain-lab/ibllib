...
def plot_brain_surface_points(brain_surface_points, ds=4, axes=None, labels=None, atlas=None):
    if axes is None:
        axes = plt.figure().add_subplot(projection='3d')
...
def labels_to_image(mlap, labels, ds, orig_shape):
    ...
    labels_image = griddata(np.stack([ml, ap], axis=1), labels, (ml_grid, ap_grid), method='nearest')
