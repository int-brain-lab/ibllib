...
def create_fake_session_folder(root_data_path, lab='fakelab', mouse='fakemouse', date='1900-01-01', num='001', increment=True):
    ...
    if session_path.exists() and increment:
        ...
        return create_fake_session_folder(root_data_path, lab=lab, mouse=mouse, date=date, num=num, increment=increment)
    session_path = root_data_path / lab / 'Subjects' / mouse / date / num
...
def populate_raw_spikeglx(session_path, model='3B', legacy=False, user_label='my_run', n_probes=2):
    """
...
def create_fake_complete_ephys_session(
    ...
    session_path = create_fake_session_folder(root_data_path, lab=lab, mouse=mouse, date=date, num=num, increment=increment)
    ...
    create_fake_raw_behavior_data_folder(session_path, populate=True, task='passive', folder='raw_passive_data')
    fpath = Path(session_path) / 'raw_passive_data' / '_iblrig_taskSettings.raw.json'
    passive_settings = {'CORRESPONDING_EPHYS_SESSION': f'C:\\some\\root\\folder\\Subjects\\{_mouse}\\{_date}\\{_num}'}
