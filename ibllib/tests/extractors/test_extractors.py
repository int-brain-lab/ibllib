...
class TestExtractTrialData(unittest.TestCase):
    ...
    def test_get_feedbackType(self):
        # TRAINING SESSIONS
        ft = training_trials.FeedbackType(self.training_lt5['path']).extract()[0]
        self.assertEqual(ft.size, self.training_lt5['ntrials'])
        # check if no 0's in feedbackTypes
        self.assertFalse(ft[ft == 0].size > 0)
        # -- version >= 5.0.0
        ft = training_trials.FeedbackType(self.training_ge5['path']).extract()[0]
    ...
    def test_get_contrastLR(self):
        # TRAINING SESSIONS
        cl, cr = training_trials.ContrastLR(self.training_lt5['path']).extract()[0]
        ...
        self.assertTrue(sum(~np.isnan(cl)) + sum(~np.isnan(cr)) == len(cl))
        # -- version >= 5.0.0
        cl, cr = training_trials.ContrastLR(self.training_ge5['path']).extract()[0]
        self.assertTrue(all([np.sign(x) >= 0 for x in cl if ~np.isnan(x)]))
        self.assertTrue(all([np.sign(x) >= 0 for x in cr if ~np.isnan(x)]))
        self.assertTrue(sum(np.isnan(cl)) + sum(np.isnan(cr)) == len(cl))
        self.assertTrue(sum(~np.isnan(cl)) + sum(~np.isnan(cr)) == len(cl))
    ...
    def test_get_probabilityLeft(self):
        # TRAINING SESSIONS
        pl = training_trials.ProbabilityLeft(self.training_lt5['path']).extract()[0]
        self.assertIsInstance(pl, np.ndarray)
        # -- version >= 5.0.0
        pl = training_trials.ProbabilityLeft(self.training_ge5['path']).extract()[0]
        self.assertIsInstance(pl, np.ndarray)
        ...
        *_, pLeft0, _ = biased_trials.ProbaContrasts(self.ephys['path']).extract(bpod_trials=data, settings=md)[0]
        ...
        *_, pLeft1, _ = biased_trials.ProbaContrasts(self.ephys['path']).extract(bpod_trials=data, settings=md)[0]
    ...
    def test_get_choice(self):
        # TRAINING SESSIONS
        choice = training_trials.Choice(session_path=self.training_lt5['path']).extract(save=False)[0]
        self.assertIsInstance(choice, np.ndarray)
        data = raw.load_data(self.training_lt5['path'])
        trial_nogo = np.array([~np.isnan(t['behavior_data']['States timestamps']['no_go'][0][0]) for t in data])
        if any(trial_nogo):
            self.assertTrue(all(choice[trial_nogo]) == 0)
        # -- version >= 5.0.0
        choice = training_trials.Choice(session_path=self.training_ge5['path']).extract(save=False)[0]
        self.assertIsInstance(choice, np.ndarray)
        ...
        # BIASED SESSIONS
        choice = biased_trials.Choice(session_path=self.biased_lt5['path']).extract(save=False)[0]
        self.assertIsInstance(choice, np.ndarray)
        ...
        if any(trial_nogo):
            self.assertTrue(all(choice[trial_nogo]) == 0)
        # -- version >= 5.0.0
        choice = biased_trials.Choice(session_path=self.biased_ge5['path']).extract(save=False)[0]
        self.assertIsInstance(choice, np.ndarray)
    ...
    def test_get_rewardVolume(self):
        # TRAINING SESSIONS
        rv = training_trials.RewardVolume(self.training_lt5['path']).extract()[0]
        self.assertIsInstance(rv, np.ndarray)
        # -- version >= 5.0.0
        rv = training_trials.RewardVolume(self.training_ge5['path']).extract()[0]
        self.assertIsInstance(rv, np.ndarray)
    ...
    def test_get_feedback_times_ge5(self):
        # TRAINING SESSIONS
        ft = training_trials.FeedbackTimes(self.training_ge5['path']).extract()[0]
    ...
    def test_get_feedback_times_lt5(self):
        # TRAINING SESSIONS
        ft = training_trials.FeedbackTimes(self.training_lt5['path']).extract()[0]
        self.assertIsInstance(ft, np.ndarray)
    ...
    def test_get_stimOnTrigger_times(self):
        # TRAINING SESSIONS
        sott = training_trials.StimOnTriggerTimes(self.training_lt5['path']).extract()[0]
        self.assertIsInstance(sott, np.ndarray)
        # -- version >= 5.0.0
        sott = training_trials.StimOnTriggerTimes(self.training_ge5['path']).extract()[0]
        self.assertIsInstance(sott, np.ndarray)
        # BIASED SESSIONS
        sott = biased_trials.StimOnTriggerTimes(self.biased_lt5['path']).extract()[0]
        self.assertIsInstance(sott, np.ndarray)
        # -- version >= 5.0.0
        sott = biased_trials.StimOnTriggerTimes(self.biased_ge5['path']).extract()[0]
        self.assertIsInstance(sott, np.ndarray)

    def test_stimOnOffFreeze_times(self):
        # TRAINING SESSIONS
        st = training_trials.StimOnOffFreezeTimes(self.training_lt5['path']).extract()[0]
        self.assertIsInstance(st[0], np.ndarray)

        # BIASED SESSIONS
        st = biased_trials.StimOnOffFreezeTimes(self.biased_lt5['path']).extract()[0]
        self.assertIsInstance(st[0], np.ndarray)

        # TRAINING SESSIONS
        st = training_trials.StimOnOffFreezeTimes(self.training_ge5['path']).extract()[0]
        self.assertIsInstance(st[0], np.ndarray)

        # BIASED SESSIONS
        st = biased_trials.StimOnOffFreezeTimes(self.biased_ge5['path']).extract()[0]
        self.assertIsInstance(st[0], np.ndarray)
    ...
    def test_get_response_times(self):
        # TRAINING SESSIONS
        rt = training_trials.ResponseTimes(self.training_lt5['path']).extract()[0]
        self.assertIsInstance(rt, np.ndarray)
        # -- version >= 5.0.0
        rt = training_trials.ResponseTimes(self.training_ge5['path']).extract()[0]
        self.assertIsInstance(rt, np.ndarray)
    ...
    def test_get_goCueTrigger_times(self):
        # TRAINING SESSIONS
        data = raw.load_data(self.training_lt5['path'])
        gct = np.array([tr['behavior_data']['States timestamps']['closed_loop'][0][0] for tr in data])
        self.assertIsInstance(gct, np.ndarray)
        # -- version >= 5.0.0
        gct = training_trials.GoCueTriggerTimes(self.training_ge5['path']).extract()[0]
        self.assertIsInstance(gct, np.ndarray)
        ...
        self.assertIsInstance(gct, np.ndarray)
        # -- version >= 5.0.0
        gct = biased_trials.GoCueTriggerTimes(self.biased_ge5['path']).extract()[0]
        self.assertIsInstance(gct, np.ndarray)

    def test_get_goCueOnset_times(self):
        # TRAINING SESSIONS
        gcot = training_trials.GoCueTimes(self.training_lt5['path']).extract()[0]
        self.assertIsInstance(gcot, np.ndarray)
        self.assertTrue(np.all(np.isnan(gcot)))
        self.assertTrue(gcot.size != 0 or gcot.size == 4)
        # -- version >= 5.0.0
        gcot = training_trials.GoCueTimes(self.training_ge5['path']).extract()[0]
        self.assertIsInstance(gcot, np.ndarray)
    ...
    def test_get_included_trials_lt5(self):
        # TRAINING SESSIONS
        it = training_trials.IncludedTrials(self.training_lt5['path']).extract()[0]
    ...
    def test_get_included_trials_ge5(self):
        # TRAINING SESSIONS
        it = training_trials.IncludedTrials(self.training_ge5['path']).extract()[0]
        self.assertIsInstance(it, np.ndarray)
        # BIASED SESSIONS
    ...
    def test_get_included_trials(self):
        # TRAINING SESSIONS
        it = training_trials.IncludedTrials(self.training_lt5['path']).extract(settings={'IBLRIG_VERSION': '4.9.9'})[0]
        self.assertIsInstance(it, np.ndarray)
        # -- version >= 5.0.0
        it = training_trials.IncludedTrials(self.training_ge5['path']).extract()[0]
        self.assertIsInstance(it, np.ndarray)
        ...
        # BIASED SESSIONS
        it = biased_trials.IncludedTrials(self.biased_lt5['path']).extract(settings={'IBLRIG_VERSION': '4.9.9'})[0]
        self.assertIsInstance(it, np.ndarray)
        # -- version >= 5.0.0
        it = biased_trials.IncludedTrials(self.biased_ge5['path']).extract()[0]
        self.assertIsInstance(it, np.ndarray)
    ...
    def test_load_encoder_positions(self):
        raw.load_encoder_positions(self.training_lt5['path'], settings={'IBLRIG_VERSION': '4.9.9'})
        raw.load_encoder_positions(self.training_ge5['path'])
        raw.load_encoder_positions(self.biased_lt5['path'], settings={'IBLRIG_VERSION': '4.9.9'})
        raw.load_encoder_positions(self.biased_ge5['path'])

    def test_load_encoder_events(self):
        raw.load_encoder_events(self.training_lt5['path'], settings={'IBLRIG_VERSION': '4.9.9'})
        raw.load_encoder_events(self.training_ge5['path'])
        raw.load_encoder_events(self.biased_lt5['path'], settings={'IBLRIG_VERSION': '4.9.9'})
...
class TestCameraExtractors(unittest.TestCase):
    def test_groom_pin_state(self):
        ...
        pulse_width = 0.3
        duty = 0.5
        gpio = {'indices': np.empty(n_pulses * 2, dtype=np.int32), 'polarities': np.ones(n_pulses * 2, dtype=np.int32)}
        gpio['polarities'][1::2] = -1
        aud_offset = 40.0
        audio = {'times': np.empty(n_pulses * 2), 'polarities': gpio['polarities']}
        ...
        np.testing.assert_array_almost_equal(ts_[:4], [40.0, 40.016667, 40.033333, 40.05])
        ...
        with self.assertLogs(camera.__name__, level='WARNING') as cm:
            gpio_, audio_, _ = camera.groom_pin_state(*map(deepcopy, (gpio, audio, ts)), tolerance=0.1, min_diff=5e-3)
        ...
        np.testing.assert_array_almost_equal(gpio_['times'][:4], np.array([41.0, 41.3, 42.6, 42.9]))
    ...
    def test_attribute_times(self, display=False):
        ...
        tsa = np.sort(np.append(tsa, 0.4))  # Add ambiguous front
        ...
        missing = [0, 4, 5, 7, 8, 10, 11, 13, 14, 16, 17, 19, 20, 22, 23, 25, 26, 28, 29, 31, 32, 34, 35, 37, 40, 43]
        expected[missing] = -1
        np.testing.assert_array_equal(matches, expected)
...
class TestGetBpodExtractor(unittest.TestCase):
    def test_get_bpod_extractor(self):
        ...
        extractor = get_bpod_extractor('', protocol='_trainingChoiceWorld', task_collection='raw_behavior_data')
    ...
    def test_get_bpod_custom_extractor(self):
        ...
        with (
            patch(f'{base_module}.get_bpod_extractor_class', return_value='toto'),
