...
class TestMesoscopeFOV(unittest.TestCase):
    ...
    def test_update_surgery_json(self):
        ...
        with self.assertLogs('ibllib.mpci.registration', 'ERROR'), mock.patch.object(one.alyx, 'rest', return_value=[record, {}]):
            task.update_surgery_json({'centerMM': {'ML': 0.0, 'AP': 0.0}}, normal_vector)
        # No matching surgery records
        with self.assertLogs('ibllib.mpci.registration', 'ERROR'), mock.patch.object(one.alyx, 'rest', return_value=[]):
            task.update_surgery_json(meta, normal_vector)
        # ONE offline
        one.mode = 'local'
        try:
