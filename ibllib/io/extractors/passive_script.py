...
for i, istim in enumerate(stim_numbers):
    ...
    STIMS[stim] = {"start": t_starts[i], "end": t_ends[i], "mkey": meta_key}
...
_, t_valve_open, _ = ephys_fpga._assign_events_bpod(
    bpod["times"], bpod["polarities"], ignore_first_valve=False
)
t_gabor = fttl["times"]
...
if DEBUG_PLOTS:
    ...
    # scatter plot
    plt.scatter(
        np.diff(gabor_fixtures), gb_diff_ts, c=(fixture["ids"][igabor - 1] == "G")[:-1], s=10
    )
    plt.colorbar()
