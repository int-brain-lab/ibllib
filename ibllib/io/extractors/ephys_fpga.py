...
import ibllib.io.extractors.ephys_passive as epassive
...
def get_protocol_period(session_path, protocol_number, sync, chmap):
    """
    ...
    bpod_sync = get_sync_fronts(sync, chmap['bpod'])
    ...
    if acquisition_description := session_params.read_params(session_path):
        ...
        if n_tasks != len(spacer_times):
            try:
                spacer_times = add_missing_passive_spacer(session_path, sync, chmap, spacer_times,
...
def add_missing_passive_spacer(session_path, sync, chmap, spacer_times, acquisition_description):
    """
    Detects the start time of a passive protcol using the frame2ttl spacers included in the passive protocol. Required
    for some sessions where the bpod spacer is missing in the passive protocol.
    Parameters
    ----------
    session_path : str, pathlib.Path
        The absolute session path, i.e. '/path/to/subject/yyyy-mm-dd/nnn'.
    ...
    chmap : dict
        ...
        loaded using the :meth:`FpgaTrials.load_sync` method.
    spacer_times : array
        An array of detected spacer times from the bpod trace
    acquisition_description: dict
        A dictionary containing the experiment acquisiton info

    Returns
    -------
    array
        An array of detected spacer times from the bpod trace in addition to the start of any passive
        protocol spacers
    """
