...
class BrainRegions(_BrainRegions):
    ...
    """
    def __init__(self, brainmap='Allen'):
        ...
        if brainmap.startswith('A'):
            self.mapping = None
        elif brainmap.startswith('B'):
            self._mapping_from_regions_list(BERYL)
    ...
    def _mapping_from_regions_list(self, new_map):
        ...
        ##
        mapping = np.zeros_like(self.id)  # non assigned regions are void
        mapping[iid] = self.id[iid]
        # Starting by the higher up levels in the hierarchy, assign all descendants to the remapping
