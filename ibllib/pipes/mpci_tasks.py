...
class PMD(base_tasks.MesoscopeTask, base_tasks.RegisterRawDataTask):
    ...
    def _train_denoiser(self,
        ...
        device = cfg.device
        if device == 'cpu':
            _logger.warning("Running PMD to generate training data on CPU, this will very slow")
    ...
    def _compress_data(self,
        ...
        device = cfg.device
        if device == 'cpu':
            _logger.warning("Running compression on CPU")
        ...
        _logger.info(f"processing complete. denoiser rank is {pmd_output.pmd_rank}")
        return pmd_output
    ...
    def _run_singlevideo(self,
        ...
        _logger.info(f"Saved snapshots for fov {fov_identifier}")
        _logger.info(f"Saving the PMD results in the ONE format")
