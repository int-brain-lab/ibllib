...
def transfer_files(src: Path, dst: Path, force: bool = False) -> None:
...
def load_fppc_params():
    if not load_params_dict("fppc_params"):
        create_fppc_params()
    return load_params_dict("fppc_params")
...
def create_fppc_params(force=False, silent=False):

    if Path(params.getfile("fppc_params")).exists() and not force:
        print(f"{params.getfile('fppc_params')} exists already, exiting...")
        print(Path(params.getfile("fppc_params")).exists())
        return
    if silent:
        data_folder_path = r"D:\iblrig_data\Subjects"
        remote_data_folder_path = r"\\iblserver.champalimaud.pt\ibldata\Subjects"
        chn_types = {"CHN_1": "ISO", "CHN_2": "GR", "CHN_3": "RD"}
    else:
        data_folder_path = cli_ask_default(
            r"Where's your LOCAL 'Subjects' data folder?", r"D:\iblrig_data\Subjects"
        )
        ...
        n_probes = int(cli_ask_default("How many channels are you using?", '3'))
        assert 100 > n_probes > 0, 'Please enter number between 1, 99 inclusive'
        chn_types = {}
        for i in range(n_probes):
            chn_types[f'CHN_{i:02}'] = cli_ask_options(
                f"What's the wavelength of CHN {i:02}?", ["ISO", "GR", "RD"])
    param_dict = {
        "DATA_FOLDER_PATH": data_folder_path,
        "REMOTE_DATA_FOLDER_PATH": remote_data_folder_path,
        **chn_types
    }
    params.write("fppc_params", param_dict)
    print(f"Created {params.getfile('fppc_params')}")
    print(param_dict)
    return param_dict
...
def rename_fp_files(session_folder: str) -> list:
    ...
    for o_name, n_name in zip(orig_names, new_names):
        ...
        if not new_file.exists():
            file = list(session_path.rglob(o_name))
            assert (len(file) == 1)
            shutil.move(str(file[0]), new_file)
...
def create_fp_transfer_done_flag(session_folder: str) -> None:
    session_path = Path(session_folder)
    flags.write_flag_file(session_path.joinpath("fp_data_transferred.flag"))
