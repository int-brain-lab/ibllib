...
class EphysRegisterRaw(base_tasks.DynamicTask):
    ...
    @property
    def signature(self):
        signature = {'input_files': [], 'output_files': [('probes.description.json', 'alf', True)]}
        return signature
...
class EphysSyncRegisterRaw(base_tasks.DynamicTask):
    ...
    def signature(self):
        signature = {
            ...
            'output_files': [
                ('*nidq.cbin', self.sync_collection, True),
        ...
        if np_conv_status == 1:
            ...
            for pext in self.pextra:
                ...
                ephys_files += spikeglx.glob_ephys_files(
                    self.session_path.joinpath(self.device_collection, f'{self.pname}{pext}'), ext='ch'
                )
                ephys_files += spikeglx.glob_ephys_files(
                    self.session_path.joinpath(self.device_collection, f'{self.pname}{pext}'), ext='meta'
                )
                for ef in ephys_files:
                    for typ in ['ap', 'lf']:
                        bin_file = ef.get(typ)
                ...
                wiring_file = next(
                    self.session_path.joinpath(self.device_collection, f'{self.pname}{pext}').glob('*wiring.json'), None
                )
                if wiring_file is None:
                    # See if we have the original wiring file
                    orig_wiring_file = next(
                        self.session_path.joinpath(self.device_collection, self.pname).glob('*wiring.json'), None
                    )
                    if orig_wiring_file is not None:
                        # copy wiring file to sub probe directory and add to output files
                        new_wiring_file = self.session_path.joinpath(
                            self.device_collection, f'{self.pname}{pext}', orig_wiring_file.name
                        )
...
class EphysSyncPulses(SyncPulses):
    ...
    def signature(self):
        signature = {
            'input_files': [
                ('*nidq.cbin', self.sync_collection, False),
...
class EphysPulses(base_tasks.EphysTask):
    ...
    def signature(self):
        signature = {
            ...
            'output_files': [
                (f'_spikeglx_sync.times.{pname}.npy', f'{self.device_collection}/{pname}', True) for pname in self.pname
            ]
            + [(f'_spikeglx_sync.polarities.{pname}.npy', f'{self.device_collection}/{pname}', True) for pname in self.pname]
            + [(f'_spikeglx_sync.channels.{pname}.npy', f'{self.device_collection}/{pname}', True) for pname in self.pname]
            + [('*sync.npy', f'{self.device_collection}/{pname}', True) for pname in self.pname]
            + [('*timestamps.npy', f'{self.device_collection}/{pname}', True) for pname in self.pname],
        }
...
class RawEphysQC(base_tasks.EphysTask):
    ...
    def signature(self):
        signature = {
            ...
            'output_files': [
                ('_iblqc_ephysChannels.apRMS.npy', f'{self.device_collection}/{self.pname}', True),
...
class CellQCMixin:
    ...
    def compute_cell_qc(folder_alf_probe):
        ...
        df_units, drift = ephysqc.spike_sorting_metrics(
            spikes.times, spikes.clusters, spikes.amps, spikes.depths, cluster_ids=np.arange(clusters.channels.size)
        )
        ...
        if file_labels.exists():
            ...
            df_units = pd.concat([df_units, ks2_labels['ks2_label'].reindex(df_units.index)], axis=1)
...
class SpikeSorting(base_tasks.EphysTask, CellQCMixin):
    ...
    SHELL_SCRIPT = Path.home().joinpath(f'Documents/PYTHON/iblscripts/deploy/serverpc/{_sortername}/sort_recording.sh')
        ...
        if not self.FORCE_RERUN:
            ...
            if log_file.exists():
                ...
                if packaging.version.parse(run_version) >= packaging.version.parse('1.7.0'):
                    _logger.info(f'Already ran: {log_file} found in {sorter_dir}, skipping.')
                    return sorter_dir
                else:
    ...
    def _run(self):
        ...
        qc_files = list(sorter_dir.glob('_iblqc_*.*.npy')) + list(sorter_dir.glob('_iblqc_*.*.pqt'))
        for file_qc in qc_files:
        ...
        if self.scratch_folder_run.exists():
        ...
        if self.one:
            ...
            if len(ins) != 0:
                ...
                resolved = ins[0].get('json', {'temp': 0}).get('extended_qc', {'temp': 0}).get('alignment_resolved', False)
                if resolved:
