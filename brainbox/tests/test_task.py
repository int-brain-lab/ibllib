...
class TestTask(unittest.TestCase):
    ...
    def test_responsive_units(self):
        ...
        alpha = 0.5
        sig_units, stats, p_values, cluster_ids = task.responsive_units(
            spike_times, spike_clusters, event_times, pre_time=[0.5, 0], post_time=[0, 0.5], alpha=alpha
        )
    ...
    def test_differentiate_units(self):
        ...
        alpha = 0.99
        sig_units, stats, p_values, cluster_ids = task.differentiate_units(
            spike_times, spike_clusters, event_times, event_groups, pre_time=0.5, post_time=0.5, alpha=alpha
        )
        num_clusters = np.size(np.unique(spike_clusters))
        self.assertTrue(np.sum(p_values < alpha) == np.size(sig_units))
        self.assertTrue(np.size(p_values) == np.size(cluster_ids))
    ...
    def test_roc_single_event(self):
        ...
        spike_clusters = self.test_data['spike_clusters']
        event_times = self.test_data['event_times']
        auc_roc, cluster_ids = task.roc_single_event(
            spike_times, spike_clusters, event_times, pre_time=[0.5, 0], post_time=[0, 0.5]
        )
        num_clusters = np.size(np.unique(spike_clusters))
    ...
    def test_roc_between_two_events(self):
        ...
        event_times = self.test_data['event_times']
        event_groups = self.test_data['event_groups']
        auc_roc, cluster_ids = task.roc_between_two_events(
            spike_times, spike_clusters, event_times, event_groups, pre_time=0.5, post_time=0.5
        )
        num_clusters = np.size(np.unique(spike_clusters))
    ...
    def test_generate_pseudo_blocks(self):
        blocks = task.generate_pseudo_blocks(100, factor=60, min_=20, max_=100, first5050=90)
    ...
    def test_generate_pseudo_stimuli(self):
        p_left, contrast_l, contrast_r = task.generate_pseudo_stimuli(100, contrast_set=[0.5, 1], first5050=90)
    ...
    def test_generate_pseudo_session(self):
        ...
        np.random.seed(456)
        for m in np.arange(10):
            pseudo_trials = task.generate_pseudo_session(test_trials, generate_choices=False, contrast_distribution='uniform')
            c += pseudo_trials.groupby('signed_contrast')['signed_contrast'].count().values / pseudo_trials.shape[0]
        self.assertTrue(np.all(np.round(c * 2) / 2 == np.array([1.0, 1.0, 1.0, 1.0, 2.0, 1.0, 1.0, 1.0, 1.0])))
