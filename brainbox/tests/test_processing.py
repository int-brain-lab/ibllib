...
class TestProcessing(unittest.TestCase):
    def test_sync(self):
        ...
        resamp = processing.sync(
            0.1, timeseries=cubes, interp="cubic", fillval="extrapolate"
        )
        ...
        resamp2 = processing.sync(
            0.1, timeseries=[squares, cubes], interp="cubic", fillval="extrapolate"
        )
        ...
        self.assertTrue(
            np.all(sq_errperc < err_margin) & np.all(cu_errperc < err_margin)
        )

        # Now check the numpy array behavior of sync.
        # Try running sync on the cubic times and values only.
        resamp = processing.sync(
            0.1, times=times, values=samples, interp="cubic", fillval="extrapolate"
        )
