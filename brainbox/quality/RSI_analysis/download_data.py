...
for i, (eid, probe) in enumerate(zip(eids, probes)):
    ...
    spikes, clusters = spikes_full[probe], clusters_full[probe]
    ...
    choices = one.load(eid, dataset_types=['trials.choice'])[0]
    ...
    signed_contrast = np.nan_to_num(contrast_R) - np.nan_to_num(contrast_L)

    pickle.dump(choices, open(folder + eid + probe + "_choices.p", "wb"))
    pickle.dump(channels, open(folder + eid + probe + "_channels.p", "wb"))
    pickle.dump(spikes, open(folder + eid + probe + "_spikes.p", "wb"))
    pickle.dump(clusters, open(folder + eid + probe + "_clusters.p", "wb"))
    pickle.dump(times_stimon, open(folder + eid + probe + "_times_stimon.p", "wb"))
    pickle.dump(times_feedback, open(folder + eid + probe + "_times_feedback.p", "wb"))
    pickle.dump(feedback, open(folder + eid + probe + "_feedback.p", "wb"))
    pickle.dump(signed_contrast, open(folder + eid + probe + "_signed_contrast.p", "wb"))
