...
for region in regions:
    ...
    for i, (eid, probe) in enumerate(zip(eids, probes)):
        ...
        """indizes_0 = np.where(choices == 0)
        indizes_1 = np.where(choices == 1)
        np.random.shuffle(indizes_0[0])
        np.random.shuffle(indizes_1[0])
        mixed_contrasts = np.zeros_like(signed_contrast)
        mixed_contrasts[choices == 0] = signed_contrast[indizes_0]
        mixed_contrasts[choices == 1] = signed_contrast[indizes_1]
        print(np.unique(signed_contrast, return_counts=True))
        signed_contrast = mixed_contrasts
        print(np.unique(signed_contrast, return_counts=True))"""

        mixed_choices = np.zeros_like(signed_contrast)
        for c in [1, 0.25, 0.125, 0.0625, 0]:
            indizes = np.where(np.abs(signed_contrast) == c)
            np.random.shuffle(indizes[0])
            mixed_choices[np.abs(signed_contrast) == c] = choices[indizes]

        print(np.unique(choices, return_counts=True))
        print(np.unique(mixed_choices, return_counts=True))
    ...
    pickle.dump(np.concatenate(region_save[region]), open(folder + region + "_save_stim_permuted_choice.p", "wb"))
