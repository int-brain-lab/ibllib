...
def count_leafs(node, tree):
    if node <= len(tree):
        return 1
    else:
        return count_leafs(tree[node - len(tree) - 1, 0], tree) + \
            count_leafs(tree[node - len(tree) - 1, 1], tree)
...
for i, (eid, probe) in enumerate(zip(eids, probes)):
    ...
    a_stim, b_stim = bb.singlecell.calculate_peths(spikes, clusters, quality.index[quality], times_stim)
    a_rew, b_rew = bb.singlecell.calculate_peths(spikes, clusters, quality.index[quality], times_rew)
    ...
    print(np.sum(model.distances_ > np.max(model.distances_) * 0.7))
