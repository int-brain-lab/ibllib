...
def linear_regression(spike_times, spike_clusters, event_times, event_targets,
                      pre_time=0, post_time=0.5, cross_validation='kfold', num_splits=5):
    """
    To do
    ...
    if cross_validation == 'none':
        # If no cross-validation train and test sets are the same
        cv = (i for i in [[np.arange(event_times.shape[0]), np.arange(event_times.shape[0])]])
    ...
    # Fit regression model to all neurons
    y_pred = np.zeros(event_times.shape[0])
    for train_index, test_index in cv:
        reg = LinearRegression()
        reg.fit(pop_vector[train_index], event_targets[train_index])
        y_pred[test_index] = reg.predict(pop_vector[test_index])
    return y_pred
