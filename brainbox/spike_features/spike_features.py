...
def depth(ephys_file, spks, clstrs, chnls, tmplts, unit, n_ch=12, n_ch_probe=385, sr=30000,
    ...
    # Get unit waveforms: #
    # Get unit timestamps.
    unit_spk_indxs = np.where(spks['clusters'] == unit)[0]
    ts = spks['times'][unit_spk_indxs]
    ...
    ch_depths = chnls['localCoordinates'][[ch], [1]]
