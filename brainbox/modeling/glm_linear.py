...
class LinearGLM(NeuralModel):
    ...
    def __init__(design_matrix, spk_times, spk_clu, fitting_metric='rsq',
    ...
    if model='default' and alpha is not None: assert('problem in model specification')
    self.fitting_metric = fitting_metric
    if model=='ridge':
        if alpha is None:
            print('alpha is None in ridge model. Falling back on sklearn default argument:alpha=(0.1, 1.0, 10.0)')
            alpha = (0.1, 1.0, 10.0)
        self.ridge, self.alpha = True, alpha
    else:
        self.ridge = False

    def _fit_sklearn(self, dm, binned, cells=None):
        if cells is None:
