...
#for each frame in the video, set 1 or zero corresponding to LED status
 ...
 d={}
 vids=['_iblrig_bodyCamera.raw.avi','_iblrig_rightCamera.raw.avi']#'_iblrig_leftCamera.raw.avi',
...
def plot_camera_sync(d,sync):
 ...
 for vid in d:
  r3=[1 if x>np.mean(d[vid][0]) else -1 for x in d[vid][0]] #threshold brightness time-series of the camera to have it in {-1,1}

  cam_times = ephys_fpga._get_sync_fronts(sync, y[vid])['times'] #get 30 Hz fpga cam time stamps
