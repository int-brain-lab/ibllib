...
def frame_smoothing(frames, window=None, window_length=None, window_type=None, sigma=1):
   ...
   smoothed_signal = np.zeros(np.max(frames) + (window_length + 1) // 2)
   for frame in frames:
       smoothed_signal[frame - window_length // 2: frame + (window_length + 1) // 2] += window
   return smoothed_signal
def smoothing_times(times, frame_length, window=None, window_type=None, window_length=None,
                   sigma=1, initial_offset=0):
   """
